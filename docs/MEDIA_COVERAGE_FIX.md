...
每个实体最多一次付费查询；相同查询并发合并，结果（包括空结果）缓存10分钟，避免反复点击产生费用。默认单进程每小时最多60次，可配置；重启清零、多实例各自计数，不能将此视为账户总账单上限。查询12秒超时，最多20候选，最多尝试3张受控CDN缩略图；不下载任意来源站点的原图，不跟随重定向。免费查找与腾讯补充总时长最多约50秒，文字行程不等待图片完成。
