...
for (const ddl of [
  ...
  'CREATE TABLE chat_runs (id INTEGER PRIMARY KEY, user_id TEXT NOT NULL, request_id TEXT NOT NULL, request_hash TEXT NOT NULL, plan_id INTEGER NOT NULL, conversation_id INTEGER NOT NULL, assistant_message_id INTEGER, status TEXT NOT NULL, steps INTEGER NOT NULL DEFAULT 0, error_code TEXT, started_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, finished_at INTEGER, UNIQUE(user_id, request_id))',
...
const cases: Record<string, () => Promise<void>> = {
  async identity() {
    const first = runs.claimRun('owner', 'request-first', 1, 1, '新增一天')
    for (const status of ['running', 'completed', 'cancelled', 'interrupted'] as const) {
      ...
      assert.throws(() => runs.claimRun('owner', 'request-first', 1, 1, '新增一天'), { statusCode: 409, data: { ...runs.publicRun(db.select().from(chatRuns).where(eq(chatRuns.id, first.id)).get()!) } })
    }
    assert.throws(() => runs.claimRun('owner', 'request-first', 1, 1, '不同需求'), { statusCode: 409 })
    assert.equal(runs.claimRun('other', 'request-first', 2, 2, '新增一天').status, 'running')
    assert.equal(db.select().from(chatRuns).all().length, 2)
  },
  async queue() {
    const first = runs.claimRun('owner', 'request-first', 1, 1, '需求')
    const queued = runs.claimRun('owner', 'request-queued', 2, 2, '需求')
    assert.equal(queued.status, 'queued')
    assert.equal(runs.claimRun('other', 'request-other', 3, 3, '需求').status, 'running')
    assert.throws(() => runs.claimRun('third', 'request-third', 4, 4, '需求'), { statusCode: 429 })
    ...
    const timeout = runs.claimRun('other', 'request-timeout', 5, 5, '需求')
  ...
  async quota() {
    ...
    const first = runs.claimRun('owner', 'request-first', 1, 1, '需求')
    runs.finishRun(first.id, 'completed')
    const second = runs.claimRun('owner', 'request-second', 1, 1, '需求')
    runs.finishRun(second.id, 'failed')
    assert.throws(() => runs.claimRun('owner', 'request-third', 1, 1, '需求'), (error: unknown) => {
    ...
    assert.equal(runs.claimRun('owner', 'request-third', 1, 1, '需求').status, 'running')
    process.env.AI_GLOBAL_REQUESTS_PER_PERIOD = '1'
    assert.throws(() => runs.claimRun('other', 'request-global', 2, 2, '需求'), { statusCode: 429 })
  },
  async checkpointRecovery() {
    ...
    const task = runs.claimRun('owner', 'request-recover', plan.planId, conversation.id, '新增一天')
    ...
    assert.throws(() => runs.claimRun('owner', 'request-recover', plan.planId, conversation.id, '新增一天'), { statusCode: 409 })
  },
  async metrics() {
