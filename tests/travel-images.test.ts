...
const mocks = vi.hoisted(() => ({ wiki: vi.fn(), search: vi.fn() }))
...
vi.mock('../server/services/cache', () => ({ getCachedBinary: async () => null, setCachedBinary: vi.fn() }))
...
beforeEach(() => { vi.clearAllMocks(); mocks.wiki.mockResolvedValue(null) })
