...
const dir = await mkdtemp(join(temporaryRoot, 'shanhai-check-'))
...
const port = await new Promise<number>((resolvePort, reject) => {
  ...
  probe.listen(0, '127.0.0.1', () => {
    ...
    if (!address || typeof address === 'string') { probe.close(); reject(new Error('无法分配隔离测试端口')); return }
...
try {
  ...
  server = Bun.spawn([process.execPath, '.output/server/index.mjs'], { env, stdout: 'inherit', stderr: 'inherit' })
  let ready = false
  for (let i = 0; i < 60; i++) {
  ...
  const checked = await realpath(dir)
  const segment = relative(temporaryRoot, checked)
  assert(segment && segment !== '..' && !segment.startsWith(`..${sep}`) && resolve(temporaryRoot, segment) === checked, '停止清理：隔离验收临时目录边界异常')
