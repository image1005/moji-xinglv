...
try {
  ...
  await step('无未捕获 UI 异常或非预期 AI/地图请求，CDN 资源保持隔离', async () => {
    ...
    assert.deepEqual(externalRequests.filter((url) => !url.startsWith('https://unpkg.com/')), [], '发现 CDN 之外的非预期外部请求，已全部阻断')
  })
} catch (error) {
  failure = error
  ...
  writeFileSync(join(artifacts, 'report.json'), JSON.stringify({
    ...
    optionalResourceImpact: externalRequests.length ? 'Markdown 编辑器尝试的 unpkg 静态扩展均已阻断；对应代码高亮、公式、图表或格式化扩展不在本轮验收范围，未实际访问 CDN。' : '未观察到外部静态资源请求。',
